...
users.post('/register', async (ctx, next) => {
  ...
  try {
    ...
    ctx.body = new ResHandleObj(200, sqlData);
  } catch (err) {
...
users.get('/checkIsLogin', async (ctx, next) =>{
  if (ctx.session && ctx.session.user) {
    ctx.body = new ResHandleObj(200, '用户处于登录状态');
  } else {
    ctx.body = new ResHandleObj(400, '当前用户没有登录');
  }
})
...
users.post('/login', async (ctx, next) => {
  ...
  if (sqlData && sqlData.length > 0) {
    ...
    ctx.body = new ResHandleObj(400, `登陆账户或密码错误`);
  }
})
...
users.post('/logout', async (ctx) => {
  ...
  ctx.body = new ResHandleObj(200, `用户退出登录`);
});
